...
/// Records which browser `blaude browser setup` last configured, so detection
...
pub async fn ensure_browser_setup_for(target: BrowserDetection) -> Result<String> {
    ...
    {
        log.push_str(
            "Override with `blaude browser setup <firefox|chrome|edge|brave|chromium|safari>` or JCODE_BROWSER.\n",
        );
    }
    ...
    if outdated {
        log.push_str(&format!(
            "Browser bridge is connected, but the live {} extension is out of date for this blaude build. Attempting repair steps...\n",
            name
        ));
    ...
    if connected && !outdated {
            ...
            || !connected_matches(&initial_status, kind)
        {
                ...
                Ok(msg) => {
                    ...
                    match wait_for_ready_for(&target, wait_secs).await {
                        ...
                        Ok(false) => {
                            ...
                            log.push_str(&format!(
                                "       Finish the steps above, then re-run `blaude browser setup {}`.\n",
                                kind.id()
                            ));
                        }
            ...
            log.push_str(&format!(
                "       Make sure {} is running with the Browser Agent Bridge extension enabled, then re-run `blaude browser status`.\n",
                name
            ));
        }
    }
    ...
    if final_status.ready {
        ...
        log.push_str(&format!("\nSetup is not complete yet. The {} extension is connected, but it is still missing required actions for this blaude build.\n", name));
        ...
        log.push_str(&format!(
            "Use `blaude browser status` to verify readiness after updating the extension in {}.\n",
            name
        ));
        ...
        log.push_str(
            "Use `blaude browser status` to re-check readiness after any manual browser step.\n",
        );
    } else {
...
fn native_host_manifest_json(kind: BrowserKind, host_path: &str) -> serde_json::Value {
    let mut manifest = serde_json::json!({
        ...
        "description": "Native host for Browser Agent Bridge (managed by blaude)",
        "path": host_path,
        "type": "stdio",
    });
...
async fn install_safari_extension() -> Result<String> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        anyhow::ensure!(
            has_xcode,
            "Xcode is required to package Safari extensions. Install Xcode from the App Store, run `xcode-select --install`, then re-run `blaude browser setup safari`."
        );
