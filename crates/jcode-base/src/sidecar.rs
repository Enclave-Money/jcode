...
const CLAUDE_CODE_JCODE_NOTICE: &str = "You are blaude, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider blaude supports:
    ...
    ///
    /// This is the universal path: it works for every provider blaude supports,
...
///
/// The "You are Claude Code" identity spoof and blaude notice are only valid
...
mod tests {
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp blaude home");
    ...
    /// Every provider blaude supports should drive the sidecar end-to-end via the
