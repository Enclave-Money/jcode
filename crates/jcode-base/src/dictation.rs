...
const CLIENT_TITLE_PREFIXES: &[&str] = &["blaude:d:", "blaude:c:"];
...
pub fn remember_last_focused_session(session_id: &str) -> Result<()> {
    ...
    std::fs::write(&path, session_id).context("failed to persist last focused blaude session")?;
...
pub fn last_focused_session() -> Result<Option<String>> {
    ...
    let session_id = match std::fs::read_to_string(path) {
        ...
        Err(err) => return Err(err).context("failed to read last focused blaude session"),
    };
    if session_id.is_empty() {
        return Ok(None);
    }
...
fn extract_session_short_name_from_window_title(title: &str) -> Option<String> {
    // Blaude windows title as "blaude …"; legacy jcode titles stay supported.
    let (_, rest) = title
        .split_once("blaude/")
        .or_else(|| title.split_once("blaude "))
