...
/// Legacy wording (blaude pre-merge); kept so persisted transcripts still
/// classify it as a synthetic continuation.
const LEGACY_TODO_COMPLETION_CONTINUATION_MESSAGE_V3: &str = "[automated follow-up - not a user message] Do more validation on the work below: it has not been independently validated yet. Keep the todo up to date; do not reply or wait for the user.";
...
/// `blaude run` auto-poke) and the transcript renderer agree on the exact text.
...
pub fn is_auto_poke_message(message: &str) -> bool {
    ...
    (trimmed.starts_with("You have ")
        ...
        || trimmed.starts_with(LEGACY_TODO_COMPLETION_CONTINUATION_MESSAGE_V3)
