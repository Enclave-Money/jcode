...
pub fn load_tokens() -> Result<GeminiTokens> {
    ...
    anyhow::bail!("No Gemini OAuth tokens found. Run `blaude login --provider gemini`.")
}
...
pub async fn login(no_browser: bool) -> Result<GeminiTokens> {
        ...
        && let Ok(listener) = super::oauth::bind_callback_listener(0)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the page says sign-in succeeded but blaude does not continue within a few seconds, press Ctrl+C and retry with `--no-browser` to use the manual code flow."
            );
