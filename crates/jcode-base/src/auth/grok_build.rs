...
pub async fn bearer_token(force_refresh: bool) -> Result<String> {
    ...
    let credential = std::fs::read(&path)
        ...
        .with_context(|| {
            format!(
                "No Grok Build login found in {}. Run `blaude login --provider grok-build`",
                path.display()
            )
        })?;
    ...
    let refresh = credential.refresh_token.as_deref().with_context(|| {
        "Grok Build login expired and has no refresh token. Run `blaude login --provider grok-build`"
    })?;
...
async fn refresh_tokens_at(
    ...
    if !status.is_success() {
        ...
        bail!(
            "Grok Build token refresh failed: {}. Run `blaude login --provider grok-build`",
            error.error_description.unwrap_or(error.error)
        );
    }
    let tokens: TokenResponse =
