...
/// The bridge loop reads the permission queue under `$JCODE_HOME`. Point it at
/// an empty home, under the crate-wide lock, so another test's queue entries
/// cannot leak into this connection's stream.
struct IsolatedHome {
...
impl IsolatedHome {
    fn new() -> Self {
        let lock = crate::jcode_home_test_lock();
