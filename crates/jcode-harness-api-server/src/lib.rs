...
/// Whoever holds this process's stdio already runs as this machine's user, so
/// it is the owner exactly as a local unix-socket client is.
...
pub(crate) async fn handle_api_io<R, W>(
    ...
    // Fail the handshake instead of serving bridge-only when the daemon is
    // down. Only the SSH/stdio entry wants this: its remote caller needs a
    // clear failure, while the socket and WebSocket doors must keep serving
    // the sign-in verbs that bring a daemon up.
    require_daemon: bool,
) -> Result<()>
where
    R: tokio::io::AsyncBufRead + Unpin,
    W: tokio::io::AsyncWrite + Unpin,
{
    ...
    // 2. Dial the legacy daemon for this client (before the capability probe,
    //    so the session connection is the first one the daemon sees). A fresh team server may not
    ...
    let (mut legacy_rx, mut legacy_write) = match dial_legacy(&legacy_socket).await {
        ...
        None if require_daemon => {
            anyhow::bail!("connect legacy socket {}", legacy_socket.display());
        }
        None => {
            eprintln!(
    ...
    // Deliberately NOT a hard connect: a fresh team server has no daemon until
    // its first AI account lands, and the bridge-local verbs (sign-in, login
    // jobs, invites) that fix that must still be served. With no daemon there
    // is nothing to probe, so skip the probe's timeout too.
    let session_tools_supported = if legacy_write.is_some() {
        daemon_supports_session_tools(&legacy_socket).await
    } else {
        false
    };
    ...
    let mut state = translate::BridgeState::with_crash_on_disconnect(
        client_name.starts_with("jcode-desktop-"),
    );
    ...
    // Cancel-safe: `read_frame_bytes` keeps a partially read frame in this
    // buffer when a sibling arm wins the select.
    ...
    loop {
        tokio::select! {
            ...
            n = read_frame_bytes(&mut reader, &mut api_frame) => {
                ...
                // Kept past the drop: a daemon-bound frame with no daemon is
                // answered with an error that must correlate to this request.
                let request_api_id = request["id"].as_u64().unwrap_or(0);
                drop(request);
                for out in outbound {
                    match out {
                        translate::Outbound::Legacy(value) => {
                            ...
                            match legacy_write.as_mut() {
                                ...
                                None => {
                                    let frame = ServerFrame::reply(request_api_id, ApiEvent::Error {
                                        code: ErrorCode::Internal,
            ...
            received = async {
                ...
                let large = legacy_line.capacity() > LARGE_FRAME_BYTES;
