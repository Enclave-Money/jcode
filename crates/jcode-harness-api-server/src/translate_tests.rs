...
/// `list_sessions` goes to the daemon (the only party that can read a room's
/// files on a team) and is answered when its `session_list` comes back. Upstream
/// tests that expected a direct local reply run through this: an empty daemon
/// list is exactly what the bridge-side merge starts from.
fn list_reply(state: &mut BridgeState, request: serde_json::Value) -> ApiEvent {
    ...
    assert_eq!(frames.len(), 1, "expected one API reply");
    assert_eq!(frames[0].reply_to, request["id"].as_u64());
    frames.remove(0).event
}
...
fn foreign_turn_terminal_becomes_turn_done() {
    ...
    // (Upstream's text framing closes the streamed message first: TextDone,
    // then TurnDone.)
    assert!(
        frames
            .iter()
            .any(|f| matches!(f.event, ApiEvent::TurnDone { .. })),
...
/// the placeholder the daemon creates for the connection. The daemon answers
/// the target subscribe with an `error`; that is an unknown_session error to
/// the client. A `state` naming a different session is not this connection's
/// answer (ids can collide across processes sharing a daemon), so it binds
...
fn attaching_a_dead_session_errors_instead_of_binding_the_placeholder() {
    ...
    assert!(
        state
            ...
            .is_empty(),
        "a state naming another session must not complete the attach"
    );
    let frames = state.legacy_event_to_api(&json!({
        "type": "error", "id": subscribe_id,
        "message": "session `session_gone` no longer exists (pruned, or the daemon restarted); create a new session instead",
    }));
    assert_eq!(frames.len(), 1);
    assert_eq!(frames[0].reply_to, Some(4));
    match &frames[0].event {
    ...
    assert!(
        ...
        "the connection must stay unattached after a refused attach"
    );
}
...
fn session_list_exposes_durable_edit_stats_without_phantom_sidecar_sessions() {
    ...
    jcode_harness_api::record_session_edit(&home.path.join("sessions"), "session_edits", stats)
        .unwrap();
    let mut state = BridgeState::default();
    let event = list_reply(&mut state, json!({"req":"list_sessions","id":77}));
    let ApiEvent::Sessions { sessions } = &event else {
        panic!("expected sessions")
    };
    assert_eq!(sessions.len(), 1);
    ...
    jcode_harness_api::record_session_edit(&home.path.join("sessions"), "session_edits", stats)
        .unwrap();
    let event = list_reply(&mut state, json!({"req":"list_sessions","id":78}));
    let ApiEvent::Sessions { sessions } = &event else {
        panic!("expected sessions")
    };
...
fn a_refused_attach_poisons_the_connection_until_the_client_reattaches() {
    ...
    let subscribe_id = subscribe["id"].as_u64().unwrap();
    let state_id = get_state["id"].as_u64().unwrap();
    // The daemon refuses the vanished target, then still reports the
    // placeholder it holds for the connection.
    let _ = state.legacy_event_to_api(&json!({
        "type": "error", "id": subscribe_id, "message": "session `session_gone` no longer exists",
    }));
    let _ = state.legacy_event_to_api(&json!({
...
/// Esc on an app whose turn died underneath it (a disconnect mid-turn tears the
/// turn down): the daemon's cancel finds nothing and sends ONLY `interrupted`.
/// No `done` will follow, so this is the one chance to end the app's spinner.
#[test]
fn interrupt_with_no_live_turn_still_ends_the_clients_turn() {
    let mut state = state_with_session();
    let frames = state.legacy_event_to_api(&json!({"type":"interrupted"}));
    assert!(
        frames
            .iter()
            .any(|f| matches!(f.event, ApiEvent::TurnDone { .. })),
        "a stale interrupt must end the turn, got {frames:?}"
    );
    assert!(
        frames
            .iter()
