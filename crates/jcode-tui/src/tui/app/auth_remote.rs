        ...
        if !importing && !PROVIDERS.contains(&provider) {
                ...
                .find(|entry| entry.id == provider)
            {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "{} setup on {host}\n\nThis login method is not yet supported by the native SSH login bridge. Run blaude login on that host and choose {}. No login was started on this computer and no local credentials were accessed.",
