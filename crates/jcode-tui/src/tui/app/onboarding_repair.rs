...
//!      login (`blaude auth-test --json`, `blaude provider add --api-key-stdin`,
//!      `blaude login`). The agent can add the key/login for the user itself.
//!
...
/// An external coding agent blaude can hand a repair task to. These map to the
...
during first-run onboarding. Please fix the login for the user.\n\n",
    ...
    if provider_hint.is_some() {
        brief.push_str(&format!(
            "  blaude auth-test --provider {provider} --json\n",
        ));
    } else {
        brief
            .push_str("  blaude auth-test --all-configured --json   # which provider is broken?\n");
    }
    brief.push_str("  blaude auth doctor   # human-readable, structured recovery steps\n\n");
...
/// to paste anything. Lives under the blaude home so it honors `JCODE_HOME`.
...
mod tests {
    ...
    fn brief_lists_the_non_interactive_commands_and_log_path() {
        ...
        assert!(
            brief.contains("blaude auth-test --provider openai --json"),
            "{brief}"
        );
        assert!(brief.contains("blaude login --provider openai"), "{brief}");
        assert!(brief.contains("--api-key-stdin"), "{brief}");
        assert!(brief.contains("blaude provider add"), "{brief}");
        // Points at the logs.
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("blaude auth-test --all-configured --json"),
            "{brief}"
        );
        assert!(brief.contains("blaude auth doctor"), "{brief}");
