...
fn remote_setup_detail(provider: LoginProviderDescriptor, host: &str) -> String {
    if super::PROVIDERS.contains(&provider.id) {
        ...
        format!(
            "Remote-host setup required: open blaude directly on {host} and use /login auto-import to review that host's other-tool logins. This SSH picker cannot perform Auto Import. To copy this computer's blaude-managed login instead, choose an explicit Import local entry. No local credentials are accessed here."
        )
    } else {
        format!(
            "Remote-host setup required: open blaude directly on {host} and use /login {} to configure this provider there. This authentication method is not supported by the SSH login flow. No local authentication is started and no local credentials are accessed here.",
            provider.id
        )
    }
}
...
fn import_entry(provider: &'static str, label: &str, host: &str) -> PickerEntry {
    entry(
        ...
        PickerOption {
            ...
            available: true,
            detail: format!(
                "Destination: {host}. One-time copy of your selected blaude-managed {provider} account. Confirmation required. No local credentials read yet. Existing remote logins are never overwritten."
            ),
            estimated_reference_cost_micros: None,
        },
        PickerAction::RemoteLogin {
