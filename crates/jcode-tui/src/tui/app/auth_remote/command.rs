...
fn parse_status(value: &serde_json::Value) -> Result<Reply, &'static str> {
    let rows = value["providers"]
        .as_array()
        .ok_or("Invalid remote auth status. Update blaude on the remote host.")?;
    ...
    for row in rows {
        let Some(descriptor) = row["id"].as_str().and_then(|id| {
            ...
            if row["status"].as_str() != Some("not_configured") {
                return Err(
                    "Unrecognized remote auth provider status. Update blaude on this computer.",
                );
            }
            continue;
        };
...
fn parse_reply(bytes: &[u8], operation: Operation, provider: &str) -> Result<Reply, &'static str> {
    ...
    let value: serde_json::Value = serde_json::from_slice(bytes)
        .map_err(|_| "Invalid remote login response. Update blaude on the remote host.")?;
    ...
    match (value["status"].as_str(), operation) {
        ...
        _ => Err("Unexpected remote login response. Update blaude on the remote host."),
    }
}
...
async fn execute(
    ...
    let exchange = async {
        ...
        if !status.success() {
            ...
            if operation == Operation::Status {
                return Err(
                    "Could not read remote auth status. Check remote blaude version and SSH access.",
                );
            }
            if operation == Operation::Import {
                return Err(
                    "Remote credential import was rejected or SSH failed. Existing remote credentials are never overwritten. Check remote blaude version and SSH access.",
                );
            }
            return Err(
                "Remote login was rejected or SSH failed. Check the callback, remote blaude version, and SSH access, then retry.",
            );
        }
        reply
    };
