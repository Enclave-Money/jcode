...
#[test]
fn blaude_branded_cli_launcher_is_found() {
    let temp = tempfile::tempdir().expect("tempdir");
    write_bundle(&temp.path().join("blaude.app"), "com.jcode.launcher");
    assert_eq!(
        legacy_macos_bundles(temp.path()),
        vec![temp.path().join("blaude.app")]
    );
}

#[test]
fn native_blaude_app_in_user_applications_is_never_matched() {
    let temp = tempfile::tempdir().expect("tempdir");
    write_bundle(&temp.path().join("Blaude.app"), "ai.blaude.app");
...
fn broker_lives_in_a_hidden_directory_outside_applications() {
    ...
    assert_eq!(
        dir,
        Path::new("/Users/test/.jcode/notifications/macos/blaude Notifications.app")
    );
