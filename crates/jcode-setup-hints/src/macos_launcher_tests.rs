...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/jcode",
        Path::new("/Users/test/Applications/blaude.app"),
    );
    assert!(script.contains("display alert \"blaude launch failed\""));
    assert!(script.contains("blaude setup-launcher"));
...
fn macos_launcher_refreshes_when_new_bundle_missing() {
    ...
    let app_dir = temp.path().join("blaude.app");
