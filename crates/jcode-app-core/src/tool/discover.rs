        ...
        && !sources.is_empty()
    {
        ...
        for source in sources {
            ...
            if let Some(url) = cached_url {
                out.push_str(&format!(" (blaude snapshot: {url})"));
            }
        }
    }
...
mod tests {
    ...
    fn render_details_includes_decision_brief_and_both_source_links() {
        ...
        assert!(rendered.contains("blaude snapshot: https://jcode.sh/docs/example/billing"));
    ...
    async fn fetch_details_posts_decision_context_and_returns_agent_brief() {
        ...
        assert!(rendered.contains("blaude snapshot: https://jcode.sh/docs/stripe/usage-based"));
    ...
    async fn details_executes_through_public_tool_interface() {
        ...
        assert!(
            output
                .output
                .contains("blaude snapshot: https://jcode.sh/docs/stripe/usage-based")
        );
        let metadata = output.metadata.unwrap();
