...
///
/// A blaude room daemon drives its own room browser (`room_browser`), which has
/// no Jev handoff, so handoff is off there too; otherwise the default
/// description steers the model into an action the room browser rejects.
fn browser_handoff_disabled() -> bool {
    std::env::var("JCODE_BROWSER_HANDOFF_DISABLED").is_ok_and(|value| value == "1")
        || super::room_browser::is_room_runtime()
}
...
async fn firefox_status(
    ...
    if status.ready {
        ...
        let body = if answering == name {
            ...
            format!(
                "Browser bridge is installed and responding in {} (blaude's detected browser is {}; actions run in {}).",
...
async fn ensure_firefox_ready(
    ...
    let mut message = String::from(
        "Browser automation is not ready yet. Use the browser tool with action='status' to confirm current state. Only run action='setup' or `blaude browser setup` for first-time install or repair when the bridge is not already ready.\n",
    );
    if !status.binary_installed {
...
async fn firefox_run_bridge_command(
    ...
    if !output.status.success() {
        ...
        if details.contains("Unknown action:") {
            anyhow::bail!(
                "The connected Firefox browser bridge is missing required support for action '{}'. This usually means the installed extension is older than the browser CLI expected by blaude. Use browser action='status' to confirm, then action='setup' to repair or update the extension.\n\nOriginal bridge error: {}",
                action,
                details
            );
        }
