...
impl Tool for EditTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Per-file lock first (batch subcalls in this process), then this
        // repo's write queue. Several teammates drive agents against one
        // checkout, and the read-modify-write below must not interleave with
        // theirs or the first edit is silently dropped. See `super::write_queue`.
        // Lock order is always file lock then repo lock (patch.rs does the same).
        let _lock = super::file_lock::lock(&path).await;
        let session = ctx.session_id.clone();
        let (content, new_content, applied) = super::write_queue::with_repo_write_lock(
            &path,
            Some(session.as_str()),
            |depth, holder| {
            ...
            || async {
                ...
                Ok::<_, anyhow::Error>((content, new_content, applied))
            },
        )
        .await?;
