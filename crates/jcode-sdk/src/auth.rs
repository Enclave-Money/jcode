...
fn parse_login_response(bytes: &[u8], operation: Operation) -> Result<serde_json::Value> {
    ...
    Err(failed("Invalid login response. Update blaude and retry."))
}
...
fn login_process_failure(stderr: &[u8]) -> Error {
    ...
    let message = if text.contains("OAuth state mismatch") {
        ...
        "The installed blaude executable does not support this login protocol. Update blaude and retry."
    } else if text.contains("Permission denied") {
        "blaude could not access its private login storage. Check local file permissions and retry."
        ...
        "The blaude login process failed before completing authentication. Start a new sign-in or check your blaude installation."
    };
    failed(message)
}
...
impl AuthFlow {
    pub fn start(&self) -> Result<AuthPrompt> {
        ...
        if !success || value["status"] != "pending" {
            return Err(failed(
                "Could not begin login. Check the installed blaude version and retry.",
            ));
        }
    ...
    fn execute(
        ...
        let mut child = self.command(operation).spawn().map_err(|_| {
            Error::new(
                ...
                "Could not start the local blaude login executable",
            )
        })?;
