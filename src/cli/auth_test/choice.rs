...
pub(crate) async fn auth_test_choice_plan(
    ...
    Ok(AuthTestChoicePlan::Skip(format!(
        "Skipped: {} local endpoint reported no models. Re-run `blaude auth-test --provider {} --model <local-model>` or set a default model first.",
...
async fn run_provider_tool_smoke_for_choice(
    ...
    run_auth_test_with_retry(async || {
        ...
        validate_auth_test_tool_smoke_transcript(&agent.messages()[transcript_start..], &output)
            .with_context(|| {
                format!(
                    "{} tool-enabled smoke prompt did not complete a valid real blaude tool loop",
                    choice.as_arg_value()
                )
            })?;
