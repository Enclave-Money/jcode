...
fn probe_generic_provider_auth(
    ...
    report.push_step(
        "refresh_probe",
        true,
        "Skipped: provider does not expose a dedicated refresh probe in blaude today.".to_string(),
    );
}
...
async fn probe_gemini_auth(report: &mut AuthTestProviderReport) {
    ...
    if crate::auth::gemini::uses_api_key() {
...
async fn probe_cursor_auth(report: &mut AuthTestProviderReport) {
    ...
    report.push_step(
        "refresh_probe",
        true,
        "Skipped: Cursor provider does not expose a native refresh-token probe in blaude today."
            .to_string(),
    );
}
