...
/// Best-effort: tell a running blaude server that on-disk auth has changed so it
...
fn login_cursor_flow() -> Result<()> {
    ...
    eprintln!("blaude will use the native Cursor HTTPS transport.");
...
async fn login_antigravity_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "blaude will authenticate directly with Google Antigravity; the Antigravity desktop app is not required."
    );
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, blaude will prompt for the callback URL instead."
    );
    eprintln!(
...
async fn login_gemini_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, blaude will prompt for the manual authorization code."
    );
    eprintln!(
        ...
        Err(_) => {
            ...
            match input.trim() {
                ...
                "3" => {
                    ...
                    eprintln!("   - Fill in app name (e.g. 'blaude') and your email");
