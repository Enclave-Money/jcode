    ...
    match provider_id {
        ...
        other => format!(
            "Next step: run blaude with a model available on this endpoint, for example \
             `blaude --provider {} --model <model-id> run 'hello'`.",
            other
        ),
    }
}
...
mod tests {
    ...
    fn extracted_hints_match_the_strings_login_printed_before_extraction() {
        assert_eq!(
            ...
            "Next step: install a model with `ollama pull llama3.2`, then run `blaude --provider ollama --model llama3.2 run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("lmstudio"),
            "Next step: load a chat model in LM Studio's Local Server, then run blaude with that exact model id, for example `blaude --provider lmstudio --model <model-id> run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("ollama-turbo"),
            "Next step: run blaude with a model available on this endpoint, for example `blaude --provider ollama-turbo --model <model-id> run 'hello'`."
        );
    }
