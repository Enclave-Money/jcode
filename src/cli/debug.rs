...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A blaude server is running (blaude or blaude serve)");
        eprintln!("  2. debug_socket is enabled in ~/.jcode/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'blaude debug start' to start a server.");
        eprintln!("Use 'blaude debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running blaude servers found.");
        println!("\nStart one with: blaude debug start");
        return Ok(());
    }

    println!("Running blaude servers:\n");
    ...
    println!("  blaude debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'blaude debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting blaude server...");
