...
impl SshConnection {
    async fn connect(options: &SshOptions) -> Result<Self> {
        Self::spawn(options.command(), STARTUP_TIMEOUT)
            .await
            .with_context(|| format!("connecting native blaude on {}", options.host))
    }
...
async fn verify_daemon_protocol<R: AsyncBufRead + Unpin, W: AsyncWrite + Unpin>(
    ...
    tokio::time::timeout(STARTUP_TIMEOUT, async {
        ...
        if pong["type"] != "pong" || pong["id"].as_u64() != Some(0)
            ...
            bail!("remote daemon does not support native SSH protocol {PROTOCOL}; update/reload the remote blaude server or select a matching daemon socket");
        }
        Ok::<_, anyhow::Error>(())
