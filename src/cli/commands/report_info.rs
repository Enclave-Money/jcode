...
async fn run_auth_doctor_validation(
    ...
    .await
    {
        ...
        Err(_) => format!(
            "validation timed out after {}s; run `blaude auth-test --provider {}` for detailed output",
...
pub(super) async fn run_usage_command(emit_json: bool) -> Result<()> {
    ...
    if report.providers.is_empty() {
        ...
        println!();
        println!("Next steps:");
        println!("- Use `blaude login --provider claude` to connect Claude OAuth.");
        println!("- Use `blaude login --provider openai` to connect ChatGPT / Codex OAuth.");
        return Ok(());
    }
    ...
    if let Some(provider_arg) = provider_arg {
        let provider =
            crate::provider_catalog::resolve_login_provider(provider_arg).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown provider '{}'. Use `blaude provider list` to see valid provider ids.",
                    provider_arg
                )
            })?;
...
mod tests {
    ...
    async fn cli_auth_status_doctor_and_login_lifecycle_uses_fresh_sandbox() {
        ...
        assert!(before_doctor_provider.diagnostics.iter().any(|line| {
            line == &format!("{} is not configured for blaude yet.", provider.display_name)
        }));
        assert!(
            before_doctor_provider
                ...
                .iter()
                .any(|line| {
                    line == &format!("Connect it: blaude login --provider {}", provider.id)
                })
        );
        ...
        assert!(
            after_doctor_provider
                .recommended_actions
                .iter()
                .any(|line| { line == "Review current state: blaude auth status --json" })
        );
    }
}
