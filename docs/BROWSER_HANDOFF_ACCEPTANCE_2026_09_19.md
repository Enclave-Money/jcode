...
Jev handoff. Three independently launched fresh blaude sessions selected handoff
...
measured BYOK speed advantage on this workload. It does not establish live blaude
...
- Each trial launched that exact binary as its own daemon with private socket
  and runtime directory, then a fresh `blaude run --ndjson` session in an empty
...
`JCODE_BROWSER_JEV_PROVIDER`. Auto selection prefers blaude credentials, checks
...
`live_subscription_jev_decision_smoke` test. It stopped at missing blaude credentials,
...
1. Sign in to an eligible account with `blaude account login`.
