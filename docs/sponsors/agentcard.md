# AgentCard: implement blaude CLI attribution
...
```text
Implement blaude referral attribution in the AgentCard admin CLI signup flow.

Requirements:
1. Accept `--via <source>` on the account-creating signup command. The blaude
...
## blaude catalog setup

blaude must serve the final production signup command directly in AgentCard's
...
Do not make blaude fetch and execute mutable third-party documentation at
runtime. Keep the short attribution-bearing command in blaude's hosted catalog,
...
## Validate against live blaude

After deploying the AgentCard CLI and updating blaude's hosted catalog:
...
blaude binaries. It must report AgentCard's CLI attribution as `attributed` and
...
1. Start blaude in a clean test environment.
...
The HTTP benchmark proves blaude serves the contract. The clean signup proves
