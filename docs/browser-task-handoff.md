...
blaude owns the task state and execution loop. Jev selects the next action from the
...
blaude executes the selected action, observes its result, appends that evidence, and
