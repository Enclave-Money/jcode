...
Sign in with `blaude account login` for subscription access. The browser client
prefers the blaude subscription credential, verifies the live `/v1/me`
...
`typesafe`, or `aimlapi`. Its default is `auto`: blaude, then OpenRouter, TypeSafe,
...
For OpenRouter BYOK, connect using `blaude login openrouter`. Credentials remain
...
chat completions, and uses that key's credits and cap. blaude does not buy credits
...
the blaude gateway. A screenshot returned to the parent cannot be text-redacted.
...
This test refuses any non-blaude route and requires a real typed decision. For
...
reuse a prior local blaude fixture, and use an existing browser session:
...
blaude debug --socket "$SHARED" sessions
blaude debug --socket "$SHARED" clients:map
blaude debug --socket "$SHARED" background:tasks
blaude debug --socket "$SHARED" jobs
blaude debug --socket "$SHARED" server:info
```
...
```bash
blaude server promote <installed-version> --json
blaude server reload --json
```
