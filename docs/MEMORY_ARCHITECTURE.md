...
blaude keeps persistent memories locally and sends bounded batches directly to
...
| blaude subscription | Existing blaude login | Trusted blaude gateway `/v1/decisions` |

Environment files use the existing blaude provider-config directory and
`KEY=value` format. OpenRouter can also be connected using `blaude login openrouter`.
...
`auto` chooses the first configured credential route in this order: blaude,
...
without the capability fail closed. With a blaude login configured, `auto` still
selects blaude on an older gateway. To use BYOK in that situation, explicitly set
...
through blaude's gateway to its configured Jev upstream. Do not store secrets in
