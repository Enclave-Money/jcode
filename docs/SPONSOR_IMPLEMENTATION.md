...
This is the sponsor-facing entry point for implementing and validating blaude
...
- blaude owns the select-phase `setup` text that agents follow.
- The sponsor's docs remain the authority for installing and using the product.
- blaude's setup must include the attribution-bearing signup command directly.
...
2. The live blaude select response includes the exact configured marker.
...
4. A clean end-to-end signup through a release blaude binary creates a sponsor
...
command blaude would hand to an agent, executes that command through the public
