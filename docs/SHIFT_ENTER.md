...
blaude requests the protocol at startup (`enable_keyboard_enhancement`) and
...
Inside tmux, blaude also requests xterm `modifyOtherKeys` mode 2
...
pane. With `extended-keys-format csi-u`, Shift+Enter then reaches blaude as
`ESC[13;2u`. blaude reasserts the request when reapplying terminal modes and resets
it with `ESC[>4;0m` on cleanup. `extended-keys always` is not required.

Three situations still break, and blaude handles each explicitly:
...
- **Trailing backslash then Enter** inserts a newline, matching shell line
  continuation. The first time you use it, blaude points you at
  `/terminal-setup`.
