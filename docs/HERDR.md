...
blaude has built-in terminal routing for Herdr. When a headed session launch is requested from a client with `HERDR_ENV=1` and `HERDR_PANE_ID`, blaude splits the calling pane to the right, focuses the new pane, and starts the resumed blaude session there. `HERDR_BIN_PATH` is honored when present.
...
The sequence must be monotonically increasing for the source. Map blaude hook sources as follows where possible:
...
```text
blaude --resume <agent_session_id>
```

blaude session IDs are opaque strings and fit Herdr's ID-based session reference model. No transcript path is needed.
...
1. Add `blaude` to `IntegrationTarget`, CLI parsing, labels, command discovery, recommendations, status, install, and uninstall handling.
2. Install a config-safe blaude session hook adapter without overwriting an existing user hook. If Herdr cannot safely compose the single blaude hook command, coordinate a small multi-hook or native-emitter addition in blaude first.
3. Accept `("herdr:blaude", "jcode")` as an official session source.
4. Persist its ID session reference and map it to `blaude --resume <id>` during restore.
5. Add blaude process detection and a bundled screen manifest for idle, working, and blocked UI states.
6. Keep screen-manifest detection authoritative until blaude exposes complete blocked, approval-result, interrupt, and exit transitions.
