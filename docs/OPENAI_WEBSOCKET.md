...
blaude's native OpenAI providers (`openai` and `openai-api`) prefer persistent
...
When an idle client subscribes, blaude snapshots its tools and static instructions
...
output. If the warmup finishes before generation is needed, blaude continues on
