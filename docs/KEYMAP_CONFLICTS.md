...
blaude runs inside a terminal, which runs inside an OS. Both layers can claim a
key chord before it ever reaches blaude (for example Ghostty binding `Ctrl+Tab`
to "next tab", or macOS binding `Cmd+Space` to Spotlight). When that happens, a
blaude keybinding silently does nothing and it is not obvious why.

This feature discovers the key bindings that exist on the machine, compares them
against blaude's own configured bindings, and warns about overlaps.
...
It is a snapshot taken at startup, not a live hook, so changes made while blaude
...
- **Startup notice.** On launch, if the set of conflicts has *changed* since the
  last time we warned, blaude shows a one-time heads-up pointing at `/keys`. It is
...
The report names the conflicting blaude action and its config field, e.g.:

```
  ⚠ Ctrl+Tab
      blaude: Switch to next model (keybindings.model_switch_next = "ctrl+tab")
      taken by terminal: next_tab
```

To fix, either:

- rebind the blaude action in `~/.jcode/config.toml` under `[keybindings]`
...
- `chord.rs` - `KeyChord`, a normalized `(cmd/ctrl/alt/shift + key)` that unifies
  ...
  blaude's own binding-string grammar.
