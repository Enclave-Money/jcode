...
- blaude sessions switch the current workspace/client to that session.
- Importable external sessions are first converted to a blaude session, then resumed in place.
...
- If terminal launch is unavailable, the UI prints manual `blaude --resume <id>` commands.
