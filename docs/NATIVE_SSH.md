...
Install a compatible blaude binary on both hosts and configure ordinary OpenSSH
...
```sh
blaude --ssh dev --remote-working-dir /srv/jcode
blaude --ssh dev --remote-working-dir /srv/jcode self-dev
blaude --ssh dev --resume session_remote_id
```
...
host with `blaude auth status --json`, never from the laptop's credential stores.
...
require `blaude login` on the host. Native attach never automatically copies local
...
active account is read from the laptop's blaude-managed OAuth store and sent over
...
- OpenAI and Claude **blaude-managed OAuth accounts only** are supported. Other
...
- New credential files use mode 0600 within a 0700 blaude data directory, with
...
The receiver command `blaude auth import --provider openai --stdin --json` is for
...
uses an owned `ssh -T` child to invoke `blaude server stdio` remotely. That command
...
This is deliberately separate from `blaude api --stdio`, whose harness API is used
